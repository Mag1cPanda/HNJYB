...
@interface OneRetakeViewController () < DJCameraViewControllerDelegate,
...
- (void)viewDidLoad
{
    ...
    iconArr = @[ @"danche001", @"danche002", @"danche003", @"jsz", @"IP018", @"IP018", @"add02", @"video", @"add02", @"add02", @"add02" ];
    titleArr = @[ @"车头前方照片",
                  @"车尾后方照片",
                  @"擦碰点照片",
                  @"车主驾驶证照片",
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (NSDictionary *dic in _qualifiedArr)
    {
        ...
        if ([dic[@"imagetype"] isEqualToString:@"14"] && indexPath.row == 5)
        {
            [cell.icon sd_setImageWithURL:[NSURL URLWithString:dic[@"imageurl"]] placeholderImage:[UIImage imageNamed:iconArr[indexPath.row]]];
        }
    }
